...
export class AuthService {
  ...
  async loginWithEmail(email, pass) {
    ...
    try {
        ...
        this.available(resp.user.uid, true)
        resolve(true)
      } catch (error) {
        reject(error)
      }
  ...
  available(uid: string, available: boolean) {
    const idSub = this.af.collection('usersystem', ref => ref.where('uid', '==', uid)).snapshotChanges()
    .pipe(map(resp => {
      return resp.map(info => info.payload.doc.id)
    }))
    .subscribe(id => {
      idSub.unsubscribe()
      this.af.doc(`usersystem/${id[0]}`).update({available})
    })
  }
